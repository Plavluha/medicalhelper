...
При возникновении проблем обращайтесь в сообщение группы:
		vk.me/hatiko_scripts или vk.com/plavluha
...
chgName.rank = {u8"Интерн", u8"Практикант", u8"Фельдшер", u8"Терапевт", u8"Парамедик", u8"Врач", u8"Завед. отделением", u8"Завед. госпеталем", u8"Зам.Гл.Врача", u8"Глав.Врач", u8"Министр Здравоохранения"}
...
local list_rank	= {u8"Интерн", u8"Практикант", u8"Фельдшер", u8"Терапевт", u8"Парамедик", u8"Врач", u8"Завед. отделением", u8"Завед. госпеталем", u8"Зам.Гл.Врача", u8"Глав.Врач", u8"Министр Здравоохранения"}
--chat
...
{sleep:0}
Здравствуйте, Вы хотите получить медицинскую карту? Предоставьте, пожалуйста, Ваш паспорт
/b /showpass {myID}
{pause}
...
[2]=Льгота
Хорошо, я Вас понял{sex:|а}. Вам нужно получить новую мед.карту.
Для вас она будет совершенно бесплатна! Но увы только на 7 дней
Вы согласны?
{pause}
#timeID=0
...
Жалобы на здоровье имеются? Имеются ли вредные привычки, а также аллергические реакции?
{pause}
	...
	lockPlayerControl(false)
		...
		sampRegisterChatCommand("nedeli", function(param)
			if tonumber(param) then
				...
				sampAddChatMessage("{6fe620}Успешно{FFFFFF}.", -1)
			else
				sampAddChatMessage("{ff6666}Ошибка{FFFFFF}.", -1)	
		    end
	    end)
		...
		sampAddChatMessage(string.format("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Приветствую, %s. Для активации главного меню пропишите в чат {22E9E3}/mhr.", sampGetPlayerNickname(myid):gsub("_"," ")), 0xEE4848)
		wait(200)
		if buf_nick.v == "" then 
...
function mainSet()
		...
		imgui.PushItemWidth(278);
			imgui.PushStyleVar(imgui.StyleVar.FramePadding, imgui.ImVec2(1, 3))
				if imgui.Button(fa.ICON_COG.."##1", imgui.ImVec2(21,20)) then
					...
					imgui.OpenPopup(u8"mhr | Изменение названия больницы")
				end
			imgui.PopStyleVar(1)
			imgui.SameLine(22)
			...
			imgui.PushStyleVar(imgui.StyleVar.FramePadding, imgui.ImVec2(1, 3))
				if imgui.Button(fa.ICON_COG.."##2", imgui.ImVec2(21,20)) then
					...
					imgui.OpenPopup(u8"mhr | Изменение названия должности")
				end
			imgui.PopStyleVar(1)
			imgui.SameLine(22)
	...
	imgui.PopItemWidth()
	imgui.EndGroup()
	if imgui.BeginPopupModal(u8"mhr | Изменение названия больницы", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	if imgui.BeginPopupModal(u8"mhr | Изменение названия должности", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
	...
	if mainWin.v then
		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " MedicalHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
				...
				if imgui.CollapsingHeader(u8"Ценовая политика") then
					...
					imgui.BeginGroup()
						...
						--	if imgui.InputText(u8"Обновление мед.карты", buf_upmed, imgui.InputTextFlags.CharsDecimal) then needSave = true end
							...
							if imgui.InputText(u8"Выдача антибиотиков", buf_ant, imgui.InputTextFlags.CharsDecimal) then needSave = true end
				...
				imgui.EndChild();
					if cmdBind[selected_cmd].rank <= num_rank.v+1 then
						...
						if imgui.Button(u8"Назначить клавишу", imgui.ImVec2(140, 20)) then 
							imgui.OpenPopup(u8"mhr | Установка клавиши для активации");
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.TextColoredRGB("Благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
						...
						if imgui.Button(u8"Удалить скрипт", imgui.ImVec2(160, 20)) then 
							...
							sampAddChatMessage("", 0xEE4848)
							sampAddChatMessage("", 0xEE4848)
							sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Внимание! Подтвердите удаление командой {77DF63}/mhr-delete.", 0xEE4848)
							mainWin.v = false
			...
			--///Установка клавиши
				...
				if imgui.BeginPopupModal(u8"mhr | Установка клавиши для активации", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
...
function tags(par)
		...
		if par:find("{getNickByID:%d+}") then
			for v in par:gmatch("{getNickByID:%d+}") do
				...
				if sampIsPlayerConnected(id) then
					...
					sampAddChatMessage("{FFFFFF}[{EE4848}mhr:Ошибка{FFFFFF}]: Параметр {getNickByID:ID} не смог вернуть ник игрока. Возможно игрок не в сети.", 0xEE4848)
					par = par:gsub(v,"")
				end
			end
		end
		...
		if par:find("{getNickByTarget}") then
			if targID ~= nil and targID >= 0 and targID <= 1000 and sampIsPlayerConnected(targID) then
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}mhr:Ошибка{FFFFFF}]: Параметр {getNickByTarget} не смог вернуть ник игрока. Возможно Вы не целились на игрока, либо он не в сети.", 0xEE4848)
...
function funCMD.lec(id)
	...
	if not u8:decode(buf_nick.v):find("[а-яА-Я]+%s[а-яА-Я]+") then
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Подождите-ка, сначала нужно заполнить базовую информацию. {90E04E}/mhr > Настройки > Основная информация", 0xEE4848)
		return
	end
	if id:find("%d+") then
		thread = lua_thread.create(function()
...
function funCMD.ant(id)
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				local countAnt = 1
				sampSendChat("Хорошо, стоимость одного антибиотика 25.000$. Согласны? Если согласны, то мы продолжим оформление.")
				wait(2000)
				sampSendChat("Какое количество антибиотиков Вам требуется? В час мы выдаём только 20 антибиотиков. И 5 штук за раз")
				wait(500)
				sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели равная количеству выдаваемых рецептов.", 0xEE4848)
				local len = renderGetFontDrawTextLength(font, "Выдача антибиотиков: {8ABCFA}Выбор кол-ва")
					while true do
					wait(0)
						renderFontDrawText(font, "Выдача антибиотиков: {8ABCFA}Выбор кол-ва\n{FFFFFF}[{67E56F}1{FFFFFF}] - 1 шт.\n{FFFFFF}[{67E56F}2{FFFFFF}] - 2 шт.\n{FFFFFF}[{67E56F}3{FFFFFF}] - 3 шт.\n{FFFFFF}[{67E56F}4{FFFFFF}] - 4 шт.\n{FFFFFF}[{67E56F}5{FFFFFF}] - 5 шт.", sx-len-10, sy-150, 0xFFFFFFFF)					
				...
				wait(200)
				sampSendChat("Хорошо. Сейчас выдам Вам их.")
				wait(1700)
				sampSendChat("/do На плече висит мед.сумка, в которой находятся антибиотики.")
				wait(2000)
				sampSendChat("/me открыл".. chsex("", "а") .." мед.сумку, затем взял".. chsex("", "а") .."находившиеся там антибиотики")
				wait(2000)
				sampSendChat("/me передал".. chsex("", "а") .." антибиотики человеку на против и закрыл".. chsex("", "а") .." мед.сумку")
				wait(1000)
				sampSendChat("/antibiotik "..id.." "..countAnt)
				countAnt = 1
			end)
		else
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Используйте команду /ant [id игрока].", 0xEE4848)
		end
end
...
if thread:status() ~= "dead" then 
	...
	if id:find("(%d+)") then
		thread = lua_thread.create(function()
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели, указывающую нужный пункт", 0xEE4848)
		local len = renderGetFontDrawTextLength(font, "Вакцинация: {8ABCFA}Выбор кол-ва")
		while true do
			wait(0)
			renderFontDrawText(font, "Вакцинация: {8ABCFA}Выбор варианта\n{FFFFFF}[{67E56F}1{FFFFFF}] - Тест.\n{FFFFFF}[{67E56F}2{FFFFFF}] - 1 этап.\n{FFFFFF}[{67E56F}3{FFFFFF}] - 2 этап.", sx-len-10, sy-150, 0xFFFFFFFF)					
		-- ТЕСТ
		if isKeyJustPressed(VK_1) and not sampIsChatInputActive() and not sampIsDialogActive() then
			sampSendChat("Приветствую, я "..u8:decode(chgName.rank[num_rank.v+1]).." "..u8:decode(buf_nick.v)..".")
			wait(2000)
			sampSendChat("Предоставьте вашу мед.карту пожалуйста.")
			wait(2000)
			sampSendChat("/n /showmc ".. myid)
			wait(2000)
			sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
			...
			while true do
			wait(0)
				renderFontDrawText(font, "Вакцинация: {8ABCFA}Ожидание медкарты\n{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить", sx/5*4, sy-50, 0xFFFFFFFF)
				if isKeyJustPressed(VK_RETURN) and not sampIsChatInputActive() and not sampIsDialogActive() then break end
			end
			sampSendChat("/me ".. chsex("передал", "передала") .." мед.карту обратно")
			sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели отвечающая на вопрос.", 0xEE4848)
			local len = renderGetFontDrawTextLength(font, "Вакцинация: {8ABCFA}Имеется ли заражение?")
			while true do
				wait(0)
				renderFontDrawText(font, "Вакцинация: {8ABCFA}Имеется ли заражение?\n{FFFFFF}[{67E56F}1{FFFFFF}] - Имеется.\n{FFFFFF}[{67E56F}2{FFFFFF}] - Отсутствует.", sx-len-10, sy-150, 0xFFFFFFFF)					
			--БОЛЕЕТ
			if isKeyJustPressed(VK_1) and not sampIsChatInputActive() and not sampIsDialogActive() then 
				sampSendChat("Сейчас я проведу вам тест на коронавирус.")
				wait(2000)
				sampSendChat("/do На плече висит мед.сумка.")
				wait(2000)
				sampSendChat("/me надел"..chsex("", "а").." маску и перчатки")
				wait(2000)
				sampSendChat("И так, приступим.")
				wait(2000)
				sampSendChat("/me сунул"..chsex("", "а").." руку в мед.кейс и достал"..chsex("", "а").." «ПЛР» тест на «СOVID-19»")
				wait(2000)
				sampSendChat("/me достал"..chsex("", "а").." палочку с ватным окончанием, а затем сунул"..chsex("", "а").." её в конец носа")
				wait(2000)
				sampSendChat("/me вставил"..chsex("", "а").." палочку с мазком в тест")
				wait(2000)
				sampSendChat("/me подождал"..chsex("", "а").." определённое время, а затем глянула"..chsex("", "а").." на цвет теста")
				wait(2000)
				sampSendChat("/do На тесте цвет: Красный.")
				wait(2000)
				sampSendChat("Вы больны коронавирусом, нужно лечится антибиотиками.")
				wait(2000)
				sampSendChat("Приобрести их сможете на Центральном рынке или в Медецинском центре.")
			break end
			--НЕ БОЛЕЕТ
			if isKeyJustPressed(VK_2) and not sampIsChatInputActive() and not sampIsDialogActive() then
				sampSendChat("Сейчас я проведу вам тест на коронавирус.")
				...
				sampSendChat("/do На плече висит мед.сумка.")
				wait(2000)
				sampSendChat("/me достал"..chsex("", "а").." оттуда перчатки и маску")
				wait(2000)
				sampSendChat("И так, приступим.")
				wait(2000)
				sampSendChat("/me сунул"..chsex("", "а").." руку в мед.кейс и достал"..chsex("", "а").." «ПЛР» тест на «СOVID-19»")
				...
				sampSendChat("/me вставил"..chsex("", "а").." палочку с мазком в тест")
				wait(2000)
				sampSendChat("/me подождал"..chsex("", "а").." определённое время, а затем глянул"..chsex("", "а").." на цвет теста")
				wait(2000) --1
				sampSendChat("/do На тесте цвет: Зелёный.")
				wait(2000)
				sampSendChat("Всё отлично, вы не больны этим вирусом.")
			break end
			end
		break end
		--ПЕРВЫЙ УКОЛ
		if isKeyJustPressed(VK_2) and not sampIsChatInputActive() and not sampIsDialogActive() then
			sampSendChat("/me достал"..chsex("", "а").." из подсумка стерильные латексные перчатки, затем надел"..chsex("", "а").." их")
			wait(2000)
			sampSendChat("/do На столе лежат хирургические инструменты и герметичные спиртовые салфетки.")
			wait(2000)
			sampSendChat('/me взял'..chsex("", "а")..' пачку с салфеткой, затем открыл'..chsex("", "а")..' её и выкинул'..chsex("", "а")..' пачку в урну с отходами класса "А"')
			wait(2000)
			sampSendChat("/todo Сейчас будет первый укол с вакциной*держа в руке салфетку")
			wait(2000)
			sampSendChat("/me закатав рукав пациенту обработал"..chsex("", "а").." плечо пациента, затем выкинул"..chsex("", "а").." использованную спиртовую ..")
			wait(2000)
			sampSendChat("/me .. салфетку в мусорку и достал"..chsex("", "а").." шприц, после чего вскрыл"..chsex("", "а").." его и сделал"..chsex("", "а").." забор вакцины в шприц")
			wait(2000)
			sampSendChat('/do Шприц наполнен "VacCovid-19".')
			wait(2000)
			sampSendChat("/me вколол"..chsex("", "а").." шприц в плечо человка, после чего ввел"..chsex("", "а").." вакцину")
			wait(2000)
			sampSendChat('/me после введения вакцины вытащил'..chsex("", "а")..' шприц и выкинул'..chsex("", "а")..' его в урну с отходами класса "Б"')
			wait(2000)
			sampSendChat("/todo Держите сэр*передавая ватку человеку напротив")
			wait(2000)
			sampSendChat("Через 2 минуты сделаем второй укол")
			wait(2000)
		...
		if isKeyJustPressed(VK_3) and not sampIsChatInputActive() and not sampIsDialogActive() then
			...
			sampSendChat('/me взял'.. chsex("", "а") .. ' пачку с салфеткой, затем открыл'.. chsex("", "а") .. ' её и выкинул'.. chsex("", "а") .. ' пачку в урну с отходами класса "А"')
			wait(2000)
			sampSendChat("/todo Сейчас будет второй укол с вакциной*держа в руке салфетку")
			wait(2000)
			sampSendChat("/me закатав рукав пациенту обработал"..chsex("", "а").." плечо пациента, затем выкинул"..chsex("", "а").." использованную спиртовую ...")
			wait(2000)
			sampSendChat("/me ... салфетку в мусорку и достал"..chsex("", "а").." шприц, после чего вскрыл"..chsex("", "а").." его и сделал"..chsex("", "а").." забор вакцины в шприц")
			...
			sampSendChat("/vaccine "..id)
			wait(2000)
			sampSendChat("/todo Всего Вам доброго*снимая перчатки и маску, а затем выкидывая в специальную урну.")
		break end
		end
	end)
	else
	...
	if not isCharInModel(PLAYER_PED, 416) then -- not
		...
		elseif bool then
			if stat:find(".+") then
				...
				sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Укажите обстановку, например, /mpost Спокойно.", 0xEE4848)
			end
		end
	end
end
...
if thread:status() ~= "dead" then 
	...
	if id:find("(%d+)") then
		thread = lua_thread.create(function()
		sampSendChat("Здравствуйте я "..u8:decode(buf_nick.v).." вам нужна страховка?")
		wait(2000)
		sampSendChat("Хорошо, покажите мне ваш паспорт и медкарту")
		wait(200)
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
			addOneOffSound(0, 0, 0, 1058)
			while true do
			wait(0)
				renderFontDrawText(font, "Выдача страховки: {8ABCFA}Ожидание документов\n{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить", sx/5*4, sy-50, 0xFFFFFFFF)
				if isKeyJustPressed(VK_RETURN) and not sampIsChatInputActive() and not sampIsDialogActive() then break end
			end --
		sampSendChat("Так, скажите пожалуйста,  на какой срок вам нужна страховка?")
		wait(2000)
		sampSendChat("1 неделя - 400.000, 2 недели - 800.000, 3 недели - 1.200.000")
		wait(200)
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели равная количеству недель.", 0xEE4848)
			local len = renderGetFontDrawTextLength(font, "Выдача страховки: {8ABCFA}Выбор кол-ва недель")
			while true do
				wait(0)
				renderFontDrawText(font, "Выдача страховки: {8ABCFA}Выбор кол-ва недель\n{FFFFFF}[{67E56F}1{FFFFFF}] - 1 нед.\n{FFFFFF}[{67E56F}2{FFFFFF}] - 2 нед.\n{FFFFFF}[{67E56F}3{FFFFFF}] - 3 нет.", sx-len-10, sy-150, 0xFFFFFFFF)					
		...
		wait(500)
		sampSendChat("/me взяв документы,после чего изучил данные")
		wait(2000)
		sampSendChat("/me достал нужные документы и ручку")
		wait(2000)
		sampSendChat("/me положил все на стол")
		wait(2000)
		sampSendChat("/me начал переносить данные медкарты и паспорта в документ")
		wait(2000)
		sampSendChat("/do Спустя пару минут закончил с документами.")
		wait(2000)
		sampSendChat("/me взял"..chsex("", "а").." штамп "..u8:decode(list_org_en[num_org.v+1]).." в правую руку из ящика стола и нанес"..chsex("", "ла").." оттиск в углу бланка")
		wait(2000)
		sampSendChat("/me отложив штамп в сторону и поставил"..chsex("", "а").." свою подпись, и сегодняшнюю дату")
		wait(2000)
		sampSendChat("/me передал страховку человеку напротив")
		wait(2000)
		sampSendChat("Всего хорошего не болейте. ")
		wait(2000)
		sampSendChat("/givemedinsurance "..id)
		end)
	else
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Используйте команду /ins [id игрока].", 0xEE4848)
	end
end
...
function funCMD.openupd()
	print(shell32.ShellExecuteA(nil, 'open', "пусто", nil, nil, 1))
end
function funCMD.update()
	...
	local url = "https://raw.githubusercontent.com/Plavluha/medicalhelper/main/MedicalHelper.lua"
